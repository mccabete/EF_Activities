...
```{r}
plot.run <- function(main = NA){
  sel = seq(s,ncol(ci),by=NS)
  if (!is.na(main)){
    plot(time,time,type='n',ylim=ylim,ylab="N", main = main)
...
The initial condition uncertainty begins large, but than gets smaller as time goes on. This is consistant with the table's concept endogenous stability, which should either decay or increase exponentially. This suggests that the process is internally stable. I was surprsed how quickly ic uncertainty declided. 
...
Parameter uncertainty is consistant throughout. I would expect that, unless there was intentional fusion of knowledge into the system. Right now the parameter uncertainty dominates the forecast. 
...
I would iterativly re-run the model treating one parameter at a time as known. I would then look at how the reduction of the ci of each parameter's model-run itereation scales with the full model's parameter CI.
...
The driver uncertianty increases voer time, and dominates in the future. This is pretty consistant with the table. I am pretty surprised at how small the driver uncertainty is initially. 
...
I am surprised that it declines with time. It feels like the process error should compound over time, not decrease. I guess it's more that it remains reletivly constant, and over time driver uncertainty tends to grow. 
...
lines(time2,N.I.ci[2,],lwd=0.5)
```
In this figure, site-site varibility really dominates. 
...
If I wanted to do out-of-sample predictions, I would want to better constrain site-level variabiliy. I would try to narrow down what makes sites different (lattitude? management?), and maybe see if I could explain some varibililty. To improve in-sample redictions, I would want to focus on process error. What other covariates could I be excluding? 
...
```{r}
## Ensemble estimate of IC  
plot.run()
N.I.mean = apply(N.I,2,mean)
N.I.sd = apply(N.I,2,sd)
ecoforecastR::ciEnvelope(time2,(N.I.mean + N.I.sd),(N.I.mean - N.I.sd),col=col.alpha(N.cols[1],trans))
lines(time2,N.I.ci[2,],lwd=0.5)

## Ensemble estimate of Parameter Uncertianty
plot.run()
N.IP.mean = apply(N.IP,2,mean)
N.IP.sd = apply(N.IP,2,sd)
ecoforecastR::ciEnvelope(time2,(N.IP.mean + N.IP.sd),(N.IP.mean - N.IP.sd),col=col.alpha(N.cols[2],trans))
lines(time2,N.IP.ci[2,],lwd=0.5)

## Ensemble estimate of Driver Uncertainty
plot.run()
N.IPD.mean = apply(N.IPD,2,mean)
N.IPD.sd = apply(N.IPD,2,sd)
ecoforecastR::ciEnvelope(time2,(N.IPD.mean + N.IPD.sd),(N.IPD.mean - N.IPD.sd),col=col.alpha(N.cols[3],trans))
lines(time2,N.IPD.ci[2,],lwd=0.5)

## Ensemble estimate of Process Uncertainty
plot.run()
N.IPDE.mean = apply(N.IPDE,2,mean)
N.IPDE.sd = apply(N.IPDE,2,sd)
ecoforecastR::ciEnvelope(time2,(N.IPDE.mean + N.IPDE.sd),(N.IPDE.mean - N.IPDE.sd),col=col.alpha(N.cols[4],trans))
lines(time2,N.IPDE.ci[2,],lwd=0.5)

## Ensemble estimate of random effect Uncertainty
plot.run()
N.IPDEA.mean = apply(N.IPDEA,2,mean)
N.IPDEA.sd = apply(N.IPDEA,2,sd)
ecoforecastR::ciEnvelope(time2,(N.IPDEA.mean + N.IPDEA.sd),(N.IPDEA.mean - N.IPDEA.sd),col=col.alpha(N.cols[5],trans))
lines(time2,N.IPDEA.ci[2,],lwd=0.5)
...
* Compare the Gaussian approximation to the full Monte Carlo ensemble
  ...
  Before I reduce the sample size, I can already see a difference. The assumption of a gaussian distribution really makes a difference for the driver uncertianty, and the random effects uncertainty. It both transforms the placement of the priductuve interval (Forces it to be lower especially for the random effects graph) and forces it to be smaller (especially for the driver uncertainty). Especially for the IC and parameter uncertianty, I almost couldn't tell the difference. 
...
N.little.I <- forecastN(IC=IC[prow,"N[6,30]"],  ## sample IC
                   r=param.mean["r_global"],
                   Kg=param.mean["K_global"],
                   alpha=param.mean["alpha_site[6]"],
                   beta=param.mean["beta"],
...
N.little.IP <- forecastN(IC=IC[prow,"N[6,30]"],  ## sample IC
                   r=params[prow,"r_global"],  ## sample parameters
                   Kg=params[prow,"K_global"],
                   alpha=params[prow,"alpha_site[6]"],
                   beta=params[prow,"beta"],
...
N.little.IPD <- forecastN(IC=IC[prow,"N[6,30]"],  ## sample IC
                   r=params[prow,"r_global"],  ## sample parameters
                   Kg=params[prow,"K_global"],
                   alpha=params[prow,"alpha_site[6]"],
                   beta=params[prow,"beta"],
...
N.little.IPDE <- forecastN(IC=IC[prow,"N[6,30]"],  ## sample IC
                   r=params[prow,"r_global"],  ## sample parameters
                   Kg=params[prow,"K_global"],
                   alpha=params[prow,"alpha_site[6]"],
                   beta=params[prow,"beta"],
...
N.little.IPDEA <- forecastN(IC=IC[prow,"N[6,30]"],  ## sample IC
                   r=params[prow,"r_global"],  ## sample parameters
                   Kg=params[prow,"K_global"],
                   alpha=aNew.mc,              ## sample random effect
                   beta=params[prow,"beta"],
...
## Ensemble estimate of IC  
plot.run()
N.little.I.mean = apply(N.little.I,2,mean)
N.little.I.sd = apply(N.little.I,2,sd)
ecoforecastR::ciEnvelope(time2,(N.little.I.mean + N.little.I.sd),(N.little.I.mean - N.little.I.sd),col=col.alpha(N.cols[1],trans))
lines(time2,N.I.ci[2,],lwd=0.5)

## Ensemble estimate of Parameter Uncertianty
plot.run()
N.little.IP.mean = apply(N.little.IP,2,mean)
N.little.IP.sd = apply(N.little.IP,2,sd)
ecoforecastR::ciEnvelope(time2,(N.little.IP.mean + N.little.IP.sd),(N.little.IP.mean - N.little.IP.sd),col=col.alpha(N.cols[2],trans))
lines(time2,N.IP.ci[2,],lwd=0.5)

## Ensemble estimate of Driver Uncertainty
plot.run()
N.little.IPD.mean = apply(N.little.IPD,2,mean)
N.little.IPD.sd = apply(N.little.IPD,2,sd)
ecoforecastR::ciEnvelope(time2,(N.little.IPD.mean + N.little.IPD.sd),(N.little.IPD.mean - N.little.IPD.sd),col=col.alpha(N.cols[3],trans))
lines(time2,N.IPD.ci[2,],lwd=0.5)

## Ensemble estimate of Process Uncertainty
plot.run()
N.little.IPDE.mean = apply(N.little.IPDE,2,mean)
N.little.IPDE.sd = apply(N.little.IPDE,2,sd)
ecoforecastR::ciEnvelope(time2,(N.little.IPDE.mean + N.little.IPDE.sd),(N.little.IPDE.mean - N.little.IPDE.sd),col=col.alpha(N.cols[4],trans))
lines(time2,N.IPDE.ci[2,],lwd=0.5)

## Ensemble estimate of random effect Uncertainty
plot.run()
N.little.IPDEA.mean = apply(N.little.IPDEA,2,mean)
N.little.IPDEA.sd = apply(N.little.IPDEA,2,sd)
ecoforecastR::ciEnvelope(time2,(N.little.IPDEA.mean + N.little.IPDEA.sd),(N.little.IPDEA.mean - N.little.IPDEA.sd),col=col.alpha(N.cols[5],trans))
lines(time2,N.IPDEA.ci[2,],lwd=0.5)
```
Wow. The uncertainty is totally underestimated. The difference, just between the high-N vs low-N ensemble methos is super stark for the parameter uncertainty, and the drivere uncertianty. In process uncertianty, it also looks like the "shape of the line" is also not as well captured. 
