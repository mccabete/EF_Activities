...
```{r}
## Based on Gelman plot, looks like post 500 convereged
var.burn <- window(var.out,start=500)

effectiveSize(var.burn) # Well over 5000
```
...
## Summary table
beta_1 <- summary(var.mat[,1])
beta_2 <- summary(var.mat[,2])
precision <- summary(var.mat[,3])

sum_table <- cbind(beta_1, beta_2, precision)
print(sum_table)
## plotted marginal distributions
plot(density(var.mat[,1]), main = "Beta 1")
plot(density(var.mat[,2]), main = "Beta 2")
plot(density(var.mat[,3]), main = "Precision")
...
Beta_1 and beta_2 are negativly correlated, based on the corelation coefficients and the correlation plot. The precision does not seems to be correlated to either of the betas. This sort of makes sense intuitivly. If you are trying to match a line to some data, you can get your intercept wrong and compensate with your slope, or get your slope wrong and compensate with your intercept. One effects the other. 
...
```{r}
print("Bayes Summary")
sum_table_bayes <- sum_table
print(sum_table_bayes)

print( "classical regression summary")
regression <- lm( y ~ x1 )
summary(lm( y ~ x1 ))
...
The "true" prarameters were: and intercept of 10, a slope of , and a standard deviation of 4 (Presicion of 1/(4^2) or 0.0624). The baysian regression was fraily accurate, returning a 2.08 for the slope, and a 0.061 for the precision. The intercept was consistantly underestimated. The classical regressional also returned a similar estimate of the slope (2.07) and an underestimate of the intercept. The precision/ standard error was also close: 4.042.
...
#data$b02 <- as.vector(c(0,0))      ## regression beta means
#data$Vb2 <- solve(diag(10000,2))   ## regression beta precisions
data$s1 <- 0.1                    ## error prior n/2
data$s2 <- 0.1                    ## error prior SS/2
...
var.burn <- window(var.out,start=2000)

effectiveSize(var.burn) # All babove 5000
```
...
beta_1 <- summary(var.mat[,1])
beta_2 <- summary(var.mat[,2])
beta_3 <- summary(var.mat[,3])
beta_4 <- summary(var.mat[,4])
precision <- summary(var.mat[,5])

sum_table <- cbind(beta_1, beta_2, beta_3, beta_4, precision)
print(sum_table)
...
plot(density(var.mat[,2]), main = "Beta 2")
abline(v = 2)
plot(density(var.mat[,3]), main = "Beta 3")
abline(v = -4)
plot(density(var.mat[,4]), main = "Beta 4")
abline(v = 0.5)
plot(density(var.mat[,5]), main = "Precision")
abline(v = (1/(4^2)))
...
We didn't recover the tru model perfectly. In almost every case, the central tendency of the distribution didn't exactly match the "true" value. (The exception is the relationship between the two covariates - that we were able to recover). In all cases, however, the the distribution included the "true" value. 
