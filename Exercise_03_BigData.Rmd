...
```{r}
fia <- read.csv("https://apps.fs.usda.gov/fia/datamart/CSV/RI_SEEDLING.csv", header = TRUE)
```

```{r}
species <- unique(fia$SPCD)
total <- rep(NA, length(species))
for( i in seq_along(species)){
 
   total[i] <-sum(na.omit(fia[fia$SPCD == species[i],]$TREECOUNT) )
   
  
}

abundance <- cbind(species, total)

abundance <- sort(abundance, decreasing = TRUE)

plot(sort(species, decreasing = TRUE), log(total))

```

Based on this plot, it looks like a rough rank abundance might have a rough correlaion between log abundance and rank. 
...
fluxnet <- getURL("http://fluxnet.fluxdata.org/sites/site-list-and-pages/?view=table")
flux_table <- readHTMLTable(fluxnet, header = TRUE) # Doesn't work, comes in as null
flux_table <- readHTMLList(fluxnet, header = TRUE) # Can get ehaders but no data?

flux_table <- readHTMLList(fluxnet, header = TRUE)
flux_table_parsed <- htmlParse("/Users/tess/Downloads/List_of_FLUXNET_2015_Sites.html") ## Get something with data, not sure hwo to manipulate it 
flux_table <- readHTMLTable(flux_table_parsed) #Empty

flux_table <- readHTMLTable("/Users/tess/Downloads/List_of_FLUXNET_2015_Sites.html")# Empty in same way

flux_table <- readHTMLTable("/Users/tess/Downloads/List_of_FLUXNET_2015_Sites.html",as.data.frame = TRUE)


 tableNodes = getNodeSet(flux_table_parsed, "//table")
 tb = readHTMLTable(tableNodes) # Says that tableNodes is empty and also an XMLNodeSet? 
...
Sorry to report: I couldn't for the life of me figure out how to parse this table. Everything I tried came back an empty object. The closest I got was manuualy downloading it as an html and then using html parse to make an object. From there I couldn't figure out how to turn it into a useable table or table-like object. 

If I could get the table to parse I would run somethign like this:

```
Country_IDS <- substr(fluxtable$site_id, 1, 2)
abundance <- rep(NA, length(Country_IDS))

for (i in 1:length(Country_IDS)){
grep(Country_IDS[i], fluxtable$site_id)

abundance[i] <- length(grep(Country_IDS[i], fluxtable$site_id))
}

plot(Country_IDs, abundance)
...
comments_only <- grep("#*", myCode)
myCode[comments_only]
...
wlef = nc_open("/Users/tess/Downloads/US-PFa-WLEF-TallTowerClean-2012-L0-vFeb2013.nc")
print(wlef)    ## metadata
```
...
lat <- 42.3601
lon <- 71.0589
time <- 1:30 # Chosen randomly. Not really sure how many tempurature data points we need here. Could also be the entire length of the document if you are interested in all the tempurature values in 2004. 
...
plot(doy,filter(NEE[i,],rep(1/24,24)),type='l',main=paste("Height =",height[i],"m"))
```
Comparing the MODIS graphs to the flux-tower graphs up above(but in reverse) the same overeall pattern is visible. You can see the growing season start up in the spring, and taper in the fall. 
...
```
# Chron FIA update script

- Check if FIA database subset of interest has changes
    - If yes: insert new entries into table
              - If FIA subset deleted entries (ie our databse is the only record)
                  - Quality flag local entry but do not delete
              - send report of chagnes
    - If no: Do nothing/ send automated report 
 ```
 Chron sudocode
 ``` 
    MAILTO=tmccabe@bu.edu
* */ * * mon /home/scratch/dietze_lab/FIA/update_database.sh
