...
## New and differnt R's

new_r <- c(1, 1.95, 2.05, 2.5, 2.8)
n = rep(n0,NT)
lines <- as.data.frame(n)
for (i in seq_along(new_r)){
      ## vector to store results
for(t in 2:NT){
  n[t] = n[t-1] + new_r[i]*n[t-1]*(1-n[t-1]/K)
}
...
lines <- lines[,-1]
names(lines) <- new_r
plot(time,lines[,1],ylim=c(0,12),lwd=3,type='l',
     bty='l',cex.lab=1.5,
     xlab="Time",ylab="Population Size")
lines(lines[,2],ylim=c(0,12),lwd=3,type='l',
     bty='l',cex.lab=1.5, col = "red")
lines(lines[,3],ylim=c(0,12),lwd=3,type='l',
     bty='l',cex.lab=1.5, col = "blue")
lines(lines[,4],ylim=c(0,12),lwd=3,type='l',
     bty='l',cex.lab=1.5, col = "green")
lines(lines[,5],ylim=c(0,12),lwd=3,type='l',
     bty='l',cex.lab=1.5, col = "orange")



```
As r gets larger, the point of time where the the population asymptotes moves sooner in time, and the oscillation around that asymptote become more extreme. 
...
I chose Wilcoxon, because I don't think I've ever heard of it before. 

```{r}
## PDF
x = seq(-5,5,by=0.1)
plot(x,dwilcox(x, 5,5),type='l')  		## that’s a lowercase “L” for “line”
				          	## add a line to indicate the mean (“v” is for “vertical”)
lines(x,dwilcox(x,7,5),col=2)			## try changing the mean (“col” sets the color)

lines(x,dwilcox(x,10,5),col=3)	## try changing the mean and standard dev
...
```{r}
seqx<- 1:30
plot(n[11,], x= seqx, type = "l", ylim=c(0, 15), col = "red")
lines(n[800,], x= seqx, col = "red")
lines(n[700,], x= seqx, col = "red")
lines(n[600,], x= seqx, col = "red")
lines(n[500,], x= seqx, col = "red")
lines(n[400,], x= seqx, col = "red")
lines(n[300,], x= seqx, col = "red")
lines(n[200,], x= seqx, col = "red")
lines(n[100,], x= seqx, col = "red")
lines(n[1000,], x= seqx, col = "red")
```
...
```{r}
hist(n[,15], main = "Population Forecast at t = 15", xlab = "Population")
...
## Figuring out the median trejectory
tmp <- n[,15]
tmp <- tmp[-length(tmp)]# Note: Even number of runs.
med <- median(tmp) 
index <- n[,15] == med
index <- grep("TRUE", index)
med_through_time <- n[index, ]


ylo <- rep(NA, 30)
## Getting ylo trajectory
for (i in 1:30){
  ylo[i] <- quantile(n[,i], 0.025)
}

## getting yhi tragectory
yhi <- rep(NA, 30)
for (i in 1:30){
  yhi[i] <- quantile(n[,i], 0.975)
}



plot(med_through_time) ## Check that the median looks good.

plot(med_through_time, x= seqx, type = "l", ylim=c(0, 15))
lines(n[800,], x= seqx, col = "red")
lines(n[700,], x= seqx, col = "red")
lines(n[600,], x= seqx, col = "red")
lines(n[500,], x= seqx, col = "red")
...
plot(med_through_time, x= seqx, type = "l", ylim=c(0, 15))
lines(ylo, x= seqx, col = "blue")
lines(yhi, x= seqx, col = "blue")
#ciEnvelope(med_through_time, ylo = ylo, yhi = yhi) <- The CI envelope looks bad for some reason. Am I not feeding it the right inputs? 
```
...
return(as.data.frame(cbind(med_through_time, yhi, ylo)))
}

test_0.3 <- get_pop_plot(0.3)
test_1.95 <- get_pop_plot(1.95)
test_2.05 <- get_pop_plot(2.05)
test_2.8 <- get_pop_plot(2.8)

plot(test_0.3$med_through_time, x= seqx, type = "l", ylim=c(0, 15), col = "blue", ylab= "Population", xlab = "Time")
lines(test_0.3$ylo, x= seqx, col = "blue")
lines(test_0.3$yhi, x= seqx, col = "blue")
lines(test_1.95$med_through_time, x= seqx, col = "red")
lines(test_1.95$ylo, x= seqx, col = "red")
lines(test_1.95$yhi, x= seqx, col = "red")
lines(test_2.05$med_through_time, x= seqx, col = "green")
lines(test_2.05$ylo, x= seqx, col = "green")
lines(test_2.05$yhi, x= seqx, col = "green")
lines(test_2.8$med_through_time, x= seqx, col = "purple")
lines(test_2.8$ylo, x= seqx, col = "purple")
lines(test_2.8$yhi, x= seqx, col = "purple")
