...
mosquito <- read.csv("/Users/tess/Documents/work/Ecoforecast_activities/data/Mosquito.csv", header = TRUE)
library(tidyverse)
mosquito$replicate <- as.character(mosquito$replicate)
ggplot(mosquito)+ 
  geom_point(aes(time, density, color = replicate))+
  geom_line(aes(time, density, color = replicate))
...
It was super hard for me to think of an informed prior for this data. I'd already looked at it (the plot above), and don't know much about it. I added a zero to the precision in hopes that that would balance it out.

```{r}

sigma = 5  ## prior standard deviation
data = list(X = c(mosquito$density), ## data
            N = 50,
            mu0 = 53, ## prior mean
            s1 = sigma^2, ## prior precision
            s2 = 2, 
            #S = 1/27,
            T1 = 1/185) ## data precision

NormalMean = "
model {
S ~ dgamma(s1,s2) # Prior on variance
mu ~ dnorm(mu0,T1) # prior on the mean 
for(i in 1:N){
...
"
inits <- list()
inits[[1]] <- list(mu = mean(mosquito$density))
inits[[2]] <- list(mu = (1/2)*mean(mosquito$density))
inits[[3]] <- list(mu = (1.5)*mean(mosquito$density))
...
burnin = 3400                                ## determine convergence
jags.burn <- window(jags.out,start=burnin)  ## remove burn-in
plot(jags.burn)      

acfplot(jags.burn)

effectiveSize(jags.burn) # well over 5000
...
quant <- as.numeric(quantile(jags.mat[,2], probs = c(0.975, 0.025)))
#quant_var <- as.numeric(quantile(jags.mat[,2]+jags.mat[,1], probs = c(0.975, 0.025)))

plot(density(jags.mat[,2]), main = "Density of Mu", xlim = c(-4, 18)) #Seems strongly influenced by my priors from JAGS
abline(v = quant[1], col = "green")
abline(v = quant[2], col = "green")
abline(v = quant[1]-mean(jags.mat[,1]), col = "pink")
abline(v = quant[2] + mean(jags.mat[,1]), col = "pink")
...
data = list(X = c(mosquito$density), ## data
            N = 50,
            time = as.numeric(as.factor(mosquito$time)), 
            NT = 10, 
            mu0 = 53, ## prior mean
            s1 = sigma^2, ## prior precision
            s2 = 2, 
...
mu ~ dnorm(mu0,T1) # prior on the mean 
for(i in 1:N){
    X[i] ~ dnorm(Ex[time[i]] ,S) # data model
}
for(t in 1:NT){
 Ex[t] <- mu + alpha_t[t]		  ## process model (does not vary with rep)
 alpha_t[t] ~ dnorm(0,tau_t)  ## random year effect
...
burnin = 34000                                ## determine convergence
jags.burn <- window(jags.out,start=burnin)  ## remove burn-in
plot(jags.burn)      

acfplot(jags.burn)

effectiveSize(jags.burn) # all over 5000 exept mu which is very close
...
for ( t in 1:10){
  ...
  ypred[] <- rnorm(npred,ycred[g,],1/sqrt(theta["S"]))
}
...
xpred <- 0: length(mosquito$density) 					## sequence of x values we're going to
...
ycred_list <- list(ycred, ycred, ycred, ycred, ycred,ycred, ycred, ycred, ycred, ycred)
ypred_list <- list(ypred, ypred, ypred, ypred, ypred,ypred, ypred, ypred, ypred, ypred)

for(g in seq_len(nsamp)){
  for(t in 1:10){
    ycred[[t]][g,] <- alpha[t] + jags.out['mu']
    ypred[[t]][g,] <- rnorm(npred,ycred[[t]][g,],1/sqrt(theta["S"]))
  }
